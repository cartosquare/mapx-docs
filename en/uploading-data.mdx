...
- **Max 50 MB per file** on all plans (configurable via `UPLOAD_MAX_SIZE_MB`).
  Dataset size is bounded by your plan's storage: Free 200 MB, Basic 5 GB,
  Pro 20 GB, Enterprise dedicated.
- Files are stored server-side and streamed to the browser when they exceed a
  small inline threshold; TIFF files are converted to COG for tiled loading.
