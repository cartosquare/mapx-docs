...
mapx upload ./data.geojson --session <id>          # upload: GeoJSON→layer, TIFF→COG, CSV→file
...
mapx map add-tile-layer --session <id> --url "https://tiles.example.com/{z}/{x}/{y}.png"

mapx view static --session <id>                    # static embed URL (iframe/reports)
mapx remote --session <id>                         # live view URL (SSE-synced)
mapx remote --session <id> --open                  # open it in the local browser
